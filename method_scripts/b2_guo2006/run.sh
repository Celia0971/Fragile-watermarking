#!/usr/bin/env bash
# B2 Guo 2006 — end-to-end run script
set -euo pipefail
...
EMBED_INFO="$TRIAL_DIR/embed_info.json"

echo "=== B2 Guo 2006 | attack=$ATTACK rho=$RHO attack_seed=$ATTACK_SEED wm_seed=$WM_SEED ==="

echo "--- Embed ---"
python "$SCRIPT_DIR/embed.py" \
    --input "$INPUT" --output "$WM_CSV" \
    --config "$CONFIG" --info_out "$EMBED_INFO"

echo "--- Attack ($ATTACK, rho=$RHO) on watermarked DB ---"
python "$BASELINES_DIR/attacks/attack_generator.py" \
    --input "$WM_CSV" --output "$ATK_CSV" \
    --attack "$ATTACK" --rho "$RHO" --pk_col "$PK_COL" \
    --seed "$ATTACK_SEED" --info_out "$ATK_INFO"

echo "--- Detect ---"
python "$SCRIPT_DIR/detect.py" \
    --input "$ATK_CSV" --embed_info "$EMBED_INFO" \
